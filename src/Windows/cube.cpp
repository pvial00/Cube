...
class CubeRandom {
    public:
    string random (int length, int seedlength = 16, int keylength = 16, int kdfiterations = 10) {
        char n[length];
        string iv, bytes, key, nonce;
        Cube cube;
	CubeKDF kdf;
        for (int x = 0; x < length; x++) {
            iv.push_back(char(0));
        }
	BYTE seed[seedlength] = {};
	HCRYPTPROV prov;
	CryptAcquireContext(&prov, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT);
	CryptGenRandom(prov, seedlength, seed);
	nonce = kdf.genkey(n, length, kdfiterations);
	key = kdf.genkey(reinterpret_cast<char*>(seed), seedlength, kdfiterations);
        bytes = cube.encrypt(iv, key, nonce);
        return bytes;
    }
};
